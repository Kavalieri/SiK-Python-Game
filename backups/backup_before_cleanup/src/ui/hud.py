...
class HUD:
	...
	def __init__(self, screen: pygame.Surface, config: ConfigManager, game_state: GameState):
		"""
	...
	def _initialize_hud_elements(self):
		...
		# Barra superior
		self.hud_elements['top_bar'] = HUDElement('top_bar', 0, 0, self.screen_width, 60)
		...
		self.hud_elements['game_info'] = HUDElement('game_info', self.screen_width // 2 - 150, 10, 300, 40)
		self.hud_elements['level_info'] = HUDElement('level_info', self.screen_width // 2 - 100, 50, 200, 20)
		self.hud_elements['time_info'] = HUDElement('time_info', self.screen_width // 2 - 100, 75, 200, 20)
		
		# Puntuación (derecha)
		self.hud_elements['score_info'] = HUDElement('score_info', self.screen_width - 210, 10, 200, 40)
		self.hud_elements['high_score'] = HUDElement('high_score', self.screen_width - 210, 50, 200, 20)
		self.hud_elements['combo_info'] = HUDElement('combo_info', self.screen_width - 210, 75, 200, 20)
		
		# Barra lateral derecha para powerups
		self.hud_elements['powerup_bar'] = HUDElement('powerup_bar', self.screen_width - 80, 100, 70, self.screen_height - 200)
		
		# Barra inferior para información adicional
		self.hud_elements['bottom_bar'] = HUDElement('bottom_bar', 0, self.screen_height - 40, self.screen_width, 40)
		
		# Indicadores de estado
		self.hud_elements['status_indicators'] = HUDElement('status_indicators', 10, self.screen_height - 80, 300, 30)
		
		self.logger.debug(f"HUD elements initialized: {len(self.hud_elements)} elements")
	
	def _initialize_fonts(self):
		"""Inicializa las fuentes para el HUD."""
		try:
			# Intentar cargar fuente personalizada
			font_path = self.config.get('paths', 'assets', 'assets') + '/fonts/arcade.ttf'
			...
			self.logger.warning("Fuente personalizada no encontrada, usando fuentes por defecto")
	...
	def _render_player_info(self):
		...
		# Nombre del jugador
		player_text = self.fonts['medium'].render(f"Jugador: {self.game_state.player_name}", True, self.colors['white'])
		...
		if shield_percentage > 0:
			self._render_bar(shield_element, shield_percentage, self.colors['blue'], "Escudo")
	...
	def _render_game_info(self):
		...
		level_text = self.fonts['medium'].render(f"Nivel {self.game_state.level}", True, self.colors['yellow'])
		...
		if time_remaining > 0:
			time_text = self.fonts['small'].render(f"Tiempo: {time_remaining:.1f}s", True, self.colors['white'])
	...
	def _render_score_info(self):
		...
		# Puntuación actual
		score_text = self.fonts['medium'].render(f"Puntos: {self.game_state.score:,}", True, self.colors['white'])
		self.screen.blit(score_text, (element.x, element.y))
		
		# Puntuación máxima
		high_score_element = self.hud_elements['high_score']
		high_score_text = self.fonts['small'].render(f"Récord: {self.game_state.high_score:,}", True, self.colors['yellow'])
		...
		if combo > 0:
			combo_text = self.fonts['small'].render(f"Combo: x{combo}", True, self.colors['orange'])
	...
	def _render_powerup_bar(self):
		...
		for i, powerup in enumerate(powerups[:5]):  # Máximo 5 powerups visibles
			...
			powerup_text = self.fonts['tiny'].render(powerup.name[:8], True, self.colors['green'])
	...
	def _render_bottom_bar(self):
		...
		controls_text = self.fonts['tiny'].render("WASD: Mover | Ratón: Apuntar | Clic: Disparar | ESC: Pausa", True, self.colors['light_gray'])
		self.screen.blit(controls_text, (element.x + 10, element.y + 10))
	
	def _render_bar(self, element: HUDElement, percentage: float, color: Tuple[int, int, int], label: str):
		"""
		...
		if fill_width > 0:
			pygame.draw.rect(self.screen, color,
							(element.x, element.y, fill_width, element.height))
	...
	def show_message(self, message: str, duration: float = 3.0, color: Tuple[int, int, int] = None):
		"""
	...
	def toggle_element(self, element_name: str):
		...
		if element_name in self.hud_elements:
			self.hud_elements[element_name].visible = not self.hud_elements[element_name].visible
			self.logger.debug(f"Elemento {element_name} alternado: {self.hud_elements[element_name].visible}")
