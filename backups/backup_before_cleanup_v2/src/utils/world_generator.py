...
class WorldGenerator:
	...
	def __init__(self, world_width: int, world_height: int, screen_width: int, screen_height: int):
		...
		self.logger.info(f"Generador de mundo inicializado - Mundo: {world_width}x{world_height}, Pantalla: {screen_width}x{screen_height}")
	...
	def generate_world(self) -> List[Tile]:
		"""
	...
	def generate_cluster(self, center_x: float, center_y: float, radius: float, 
						num_elements: int, element_types: List[TileType] = None) -> List[Tile]:
		"""
		...
		self.logger.debug(f"Cluster generado en ({center_x}, {center_y}) con {len(elements)} elementos")
		return elements
