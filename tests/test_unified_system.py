...
class UnifiedTestSystem:
    ...
    def __init__(self):
        ...
        self.powerups = ["potion", "shield", "sword", "coin", "heart", "star", "crystal", "ring", "scroll", "key"]
        ...
        try:
            ...
            for char_key in self.character_keys[:2]:  # Solo primeros 2 para no saturar
                for anim in animations:
                    frames = self.asset_manager.get_character_animation_frames(char_key, anim)
                    if frames:
                        self.logger.info(f"✓ {char_key} - {anim}: {len(frames)} frames")
                    else:
                        results["warnings"].append(f"{char_key} - {anim}: no disponible")
            ...
            for button_name, button_desc in self.ui_buttons:
                for state in states:
                    ...
                    if sprite:
                        ...
                        results["warnings"].append(f"Botón {button_name}_{state} no encontrado")
    ...
    def render_test_interface(self):
        ...
        # Título principal
        title_text = self.fonts['title'].render("Sistema Unificado de Pruebas", True, self.colors['text_highlight'])
        title_rect = title_text.get_rect(center=(self.screen_width // 2, 30))
        self.screen.blit(title_text, title_rect)
        
        # Subtítulo
        subtitle_text = self.fonts['subtitle'].render("SiK Python Game - Test Completo", True, self.colors['text_secondary'])
        ...
        y_offset = 100
        for test_name, result in self.test_results.items():
            status_color = self.colors['success'] if result["success"] else self.colors['error']
            status_symbol = "✓" if result["success"] else "✗"
            
            # Nombre de la prueba
            test_text = self.fonts['normal'].render(f"{status_symbol} {test_name}", True, status_color)
        ...
        if self.character_keys:
            ...
            pygame.draw.rect(self.screen, self.colors['panel_border'], char_panel_rect, 2)
            
            # Nombre del personaje
            char_name_text = self.fonts['subtitle'].render(char_data['nombre'], True, self.colors['text_highlight'])
            self.screen.blit(char_name_text, (30, y_offset + 30))
            
            # Tipo
            char_type_text = self.fonts['normal'].render(char_data['tipo'], True, self.colors['text_secondary'])
        ...
        powerup_text = self.fonts['subtitle'].render("Powerups:", True, self.colors['text_highlight'])
        ...
        for powerup in self.powerups[:8]:  # Mostrar solo los primeros 8
            ...
            if sprite:
                ...
                # Nombre del powerup
                name_text = self.fonts['small'].render(powerup, True, self.colors['text'])
        ...
        inst_text = self.fonts['small'].render(instructions, True, self.colors['text_secondary'])
        inst_rect = inst_text.get_rect(center=(self.screen_width // 2, self.screen_height - 30))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    # Personaje anterior
                    self.selected_character = (self.selected_character - 1) % len(self.character_keys)
