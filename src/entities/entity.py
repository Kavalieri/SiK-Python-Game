...
class Entity(ABC):
    ...
    def __init__(
        ...
        self.config = {
            "game": {"debug": False}
        }  # Definir atributo con un valor por defecto
    ...
    def _update_animation(self, delta_time: float):
        ...
        if self.animation_timer >= 1.0:
            ...
            self.current_frame = (self.current_frame + 1) % len(
                self.animation_frames.get(self.state.value, [])
            )
    ...
    def take_damage(self, damage: float, source: "Entity" = None) -> bool:
        ...
        self.logger.debug(
            f"Entidad recibió {actual_damage} de daño. Vida restante: {self.stats.health}"
        )
        ...
        if source:
            print(f"Daño recibido de {source}")
    ...
    def heal(self, amount: float):
        ...
        if healed_amount > 0:
            self.logger.debug(
                f"Entidad curada {healed_amount} puntos. Vida actual: {self.stats.health}"
            )
            ...
            try:
                ...
                if debug_enabled:
                    debug_rect = pygame.Rect(
                        centered_x, centered_y, sprite_width, sprite_height
                    )
                    pygame.draw.rect(screen, (255, 0, 0), debug_rect, 2)
            except Exception:  # Especificar excepción
    ...
    def debug_config(self):
        debug_enabled = self.config["game"].get("debug", False)  # Ajustar acceso
        print(f"Debug habilitado: {debug_enabled}")
