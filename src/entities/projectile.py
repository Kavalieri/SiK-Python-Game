...
class Projectile(Entity):
	...
	def __init__(self, x: float, y: float, target_x: float, target_y: float, 
				 damage: float, speed: float, config: ConfigManager):
		"""
		...
		self.logger.debug(f"Proyectil creado en ({x}, {y}) hacia ({target_x}, {target_y})")
	...
	def _setup_sprite(self):
		...
		try:
			...
			pygame.draw.circle(self.sprite, (255, 255, 0), (self.width//2, self.height//2), self.width//2)
