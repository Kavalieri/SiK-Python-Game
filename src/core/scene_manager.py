...
class SceneManager:
    ...
    def change_scene(self, scene_name: str):
        ...
        self.logger.info(
            f"[SceneManager] Cambio de escena: {self.current_scene} -> {scene_name}"
        )
    ...
    def handle_event(self, event: pygame.event.Event):
        ...
        if self.current_scene:
            self.logger.debug(
                f"Enviando evento {event.type} a escena: {self.current_scene.__class__.__name__}"
            )
            self.current_scene.handle_event(event)
        else:
            self.logger.warning(
                f"Evento {event.type} recibido pero no hay escena actual"
            )
    ...
    def render(self):
        ...
        if self.current_scene:
            ...
            self.logger.warning(
                "No hay escena actual para renderizar - mostrando pantalla de carga"
            )
