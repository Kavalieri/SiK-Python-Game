...
class ConfigManager:
		...
		try:
			if self.config_file.exists():
				with open(self.config_file, 'r', encoding='utf-8') as f:
					...
					self.logger.info(f"Configuración principal cargada desde: {self.config_file}")
			else:
	...
	def _load_specific_configs(self):
		...
		try:
			...
			if not config_dir.exists():
				self.logger.warning(f"Directorio de configuración no encontrado: {config_dir}")
				return
				...
				if file_path.exists():
					try:
						with open(file_path, 'r', encoding='utf-8') as f:
							...
							self.logger.info(f"Configuración {section} cargada desde: {file_path}")
					except Exception as e:
						self.logger.error(f"Error al cargar configuración {section}: {e}")
				else:
	...
	def get_fullscreen(self) -> bool:
		"""Obtiene si está en pantalla completa."""
		display_config = self.get_display_config()
		return display_config.get("resolución", {}).get("pantalla_completa", False)
