...
class Camera:
    ...
    def __init__(
        self, screen_width: int, screen_height: int, world_width: int, world_height: int
    ):
        """
        ...
        self.logger.info(
            f"Cámara inicializada - Mundo: {world_width}x{world_height}, Pantalla: {screen_width}x{screen_height}"
        )
    ...
    def is_visible(
        self, world_x: float, world_y: float, width: float = 0, height: float = 0
    ) -> bool:
        """
        ...
        return (
            screen_x + width >= 0
            and screen_x <= self.screen_width
