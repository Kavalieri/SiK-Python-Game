...
class SaveDatabase:
    ...
    def load_game_from_database(self, slot: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            encrypted_package_json = save_record[9]
            ...
            complete_data = {
                ...
                "save_metadata": {
                    "slot": save_record[1],
                    "nombre_jugador": save_record[2],
                    "descripcion": save_record[3],
                    "nivel_actual": save_record[4],
                    "puntuacion": save_record[5],
                    "vidas": save_record[6],
                    "tiempo_jugado": save_record[7],
                    "personaje": save_record[8],
                    "creado_en": save_record[10],
                    "actualizado_en": save_record[11],
                },
            }
    ...
    def get_all_saves_info(self) -> List[Dict[str, Any]]:
        try:
            query = "SELECT slot, nombre_jugador, descripcion, nivel_actual, puntuacion, vidas, tiempo_jugado, personaje, creado_en, actualizado_en FROM partidas_guardadas ORDER BY slot"
            ...
            self.logger.error("Error obteniendo información de partidas")
            return []
    ...
    def delete_save_from_database(self, slot: int) -> bool:
        try:
            ...
            self.logger.error("Error eliminando partida del slot %d", slot)
            return False
    ...
    def migrate_pickle_to_database(
        ...
        try:
            ...
            self.logger.error("Error migrando pickle a SQLite slot %d", slot)
            return False
