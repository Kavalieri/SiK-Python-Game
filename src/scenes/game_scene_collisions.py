...
class GameSceneCollisions:
    ...
    def check_all_collisions(self):
        ...
        for projectile in scene.projectiles[:]:
            enemies_in_range = scene.enemy_manager.get_enemies_in_range(
            ...
            for enemy in enemies_in_range:
                if self.check_collision(projectile, enemy):
                    ...
                    if enemy.is_dead:
                        ...
                        scene.logger.debug(
                            f"Enemigo eliminado. Puntuación: {scene.game_state.score}"
                        )
                    break
                ...
                if self.check_collision(scene.player, enemy):
                    if enemy.is_attack_ready():
                        ...
                        if scene.player.take_damage(damage):
                            ...
                            scene.logger.debug(
                                f"Jugador dañado. Vidas: {scene.game_state.lives}"
                            )
