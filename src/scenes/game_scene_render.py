...
class GameSceneRender:
	...
	def render_scene(self):
		...
		for projectile in scene.projectiles:
			if scene.camera.is_visible(projectile.x, projectile.y, projectile.width, projectile.height):
				screen_x, screen_y = scene.camera.world_to_screen(projectile.x, projectile.y)
				projectile.render(scene.screen, (screen_x, screen_y))
		# Renderizar powerups
		for powerup in scene.powerups:
			if scene.camera.is_visible(powerup.x, powerup.y, powerup.width, powerup.height):
		...
		# Renderizar jugador
		if scene.player:
			if scene.camera.is_visible(scene.player.x, scene.player.y, scene.player.width, scene.player.height):
				screen_x, screen_y = scene.camera.world_to_screen(scene.player.x, scene.player.y)
