...
class CharacterUI:
    ...
    def _init_ui_from_config(self):
        ...
        # Fuentes
        self.fonts = {
            'title': pygame.font.Font(None, self.config_manager.get_ui_font_size("título")),
            'subtitle': pygame.font.Font(None, self.config_manager.get_ui_font_size("subtítulo")),
            'normal': pygame.font.Font(None, self.config_manager.get_ui_font_size("normal")),
            'small': pygame.font.Font(None, self.config_manager.get_ui_font_size("pequeña"))
        }
        ...
        self.colors = {
            ...
            'card_border_selected': self.config_manager.get_ui_color("tarjeta_borde_seleccionada")
        }
    ...
    def _init_buttons(self):
        ...
        self.buttons['start'] = pygame.Rect(start_x, start_y, button_width, button_height)
    ...
    def render_title(self, screen: pygame.Surface):
        ...
        title_surface = self.fonts['title'].render(title_text, True, self.colors['text_highlight'])
        title_rect = title_surface.get_rect(center=(self.screen_width // 2, 50))
        screen.blit(title_surface, title_rect)
    
    def render_character_card(self, screen: pygame.Surface, character_key: str, 
                            x: int, y: int, is_selected: bool = False):
        """
        ...
        border_color = self.colors['card_border_selected'] if is_selected else self.colors['panel_border']
        ...
        name_surface = self.fonts['subtitle'].render(name_text, True, self.colors['text'])
        name_rect = name_surface.get_rect(center=(x + card_width // 2, y + 160))
        screen.blit(name_surface, name_rect)
        
        # Renderizar tipo
        type_text = char_data['tipo']
        type_surface = self.fonts['small'].render(type_text, True, self.colors['text_secondary'])
        type_rect = type_surface.get_rect(center=(x + card_width // 2, y + 185))
        screen.blit(type_surface, type_rect)
    
    def _render_character_image(self, screen: pygame.Surface, character_key: str, 
                              x: int, y: int, size: int = 120):
        """
    ...
    def _create_character_placeholder(self, character_key: str, size: int = 120) -> pygame.Surface:
        """
    ...
    def render_character_stats(self, screen: pygame.Surface, char_data: dict, x: int, y: int):
        ...
        stats_title = self.fonts['normal'].render("Estadísticas:", True, self.colors['text_highlight'])
        ...
        for stat_name, stat_value in stats.items():
            ...
            stat_surface = self.fonts['small'].render(stat_text, True, self.colors['text'])
            screen.blit(stat_surface, (x, y + y_offset))
            y_offset += 20
    
    def render_character_skills(self, screen: pygame.Surface, char_data: dict, x: int, y: int):
        """
        ...
        skills_title = self.fonts['normal'].render("Habilidades:", True, self.colors['text_highlight'])
        screen.blit(skills_title, (x, y))
        
        y_offset = 30
        for skill in skills:
            skill_surface = self.fonts['small'].render(f"• {skill}", True, self.colors['text'])
    ...
    def render_buttons(self, screen: pygame.Surface, mouse_pos: Tuple[int, int]):
        ...
        for button_name, button_rect in self.buttons.items():
            ...
            if text:
                text_surface = self.fonts['normal'].render(text, True, self.colors['button_text'])
                text_rect = text_surface.get_rect(center=button_rect.center)
                screen.blit(text_surface, text_rect)
    
    def render_navigation_indicator(self, screen: pygame.Surface, current_index: int, total_characters: int):
        """
        ...
        for i in range(total_characters):
            ...
            color = self.colors['text_highlight'] if i == current_index else self.colors['text_secondary']
    ...
    def get_character_card_clicked(self, mouse_pos: Tuple[int, int], 
                                 character_positions: Dict[str, Tuple[int, int]]) -> Optional[str]:
        """
