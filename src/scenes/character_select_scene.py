...
class CharacterSelectScene(Scene):
    ...
    def __init__(self, screen, config, game_state, save_manager):
        ...
        self.logger.info(
            "[CharacterSelectScene] Escena de selección de personaje inicializada"
        )
    ...
    def _render_selected_character(self):
        ...
        if character_image:
            ...
            scaled_image = pygame.transform.scale(
                character_image, (image_size, image_size)
            )
            image_rect = scaled_image.get_rect(
                center=(image_x + image_size // 2, image_y + image_size // 2)
            )
    ...
    def handle_event(self, event: pygame.event.Event):
        ...
        self.logger.info(
            f"[CharacterSelectScene] Evento recibido: {event.type} - {event}"
        )
        if event.type == pygame.KEYDOWN:
            ...
            self.logger.info(
                f"[CharacterSelectScene] Click ratón: {event.button} en {event.pos}"
            )
