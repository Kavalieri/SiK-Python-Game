...
class SlotSelectionScene(Scene):
	...
	def __init__(self, screen: pygame.Surface, config: ConfigManager, game_state, save_manager):
		...
		self.logger.info('[SlotSelectionScene] Escena de selección de slots inicializada')

	def enter(self):
		super().enter()
		self.menu_manager.show_menu('save')
		self.logger.info('[SlotSelectionScene] Entrando en escena de selección de slots')

	def handle_event(self, event: pygame.event.Event):
		self.logger.debug(f"[SlotSelectionScene] Evento recibido: {event.type} - {event}")
		if event.type == pygame.KEYDOWN:
			self.logger.info(f"[SlotSelectionScene] Tecla pulsada: {event.key}")
			if event.key == pygame.K_ESCAPE:
				self.logger.info("[SlotSelectionScene] ESC presionado - volviendo al menú principal")
				self.game_state.scene_manager.change_scene('main_menu')
		elif event.type == pygame.MOUSEBUTTONDOWN:
			self.logger.info(f"[SlotSelectionScene] Click ratón: {event.button} en {event.pos}")
