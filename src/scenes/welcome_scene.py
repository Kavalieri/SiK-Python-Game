...
class WelcomeScene(Scene):
	...
	def __init__(self, screen: pygame.Surface, config: ConfigManager, game_state, save_manager):
		...
		self.menu_manager.add_callback('on_welcome_start', self._on_main_menu)
		
		self.logger.info("Escena de bienvenida inicializada")
	
	def handle_event(self, event: pygame.event.Event):
		"""Procesa eventos de Pygame."""
		self.logger.debug(f"WelcomeScene: Procesando evento {event.type}")
		self.menu_manager.update([event])
	
	def update(self):
		"""Actualiza la lógica de la escena."""
		self.logger.debug("WelcomeScene: Actualizando escena")
	
	def render(self):
		"""Renderiza la escena."""
		self.logger.debug("WelcomeScene: Renderizando escena")
		# Fondo negro
	...
	def _on_main_menu(self):
		...
		if self.scene_manager:
			...
			self.logger.warning("scene_manager no disponible para cambiar escena") 