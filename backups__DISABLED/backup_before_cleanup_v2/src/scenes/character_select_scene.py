...
class CharacterSelectScene(Scene):
    ...
    def _load_animation_frames(self):
        ...
        for char_key in self.character_keys:
            ...
            self.logger.info(
                f"Cargados {len(frames)} frames de animación para {char_key}"
            )
    ...
    def _render_title(self):
        ...
        title_text = self.fonts["title"].render(
            "Selección de Personaje", True, self.colors["text_highlight"]
        )
        title_rect = title_text.get_rect(center=(self.screen_width // 2, 40))
        self.screen.blit(title_text, title_rect)

        subtitle_text = self.fonts["subtitle"].render(
            "Navega con las flechas", True, self.colors["text_secondary"]
        )
    ...
    def _render_characters(self):
        ...
        # Nombre
        name_text = self.fonts["title"].render(
        ...
        # Tipo
        type_text = self.fonts["subtitle"].render(
    ...
    def _render_navigation_indicator(self):
        ...
        nav_surface = self.fonts["small"].render(
            nav_text, True, self.colors["text_secondary"]
        )
        ...
        instructions = (
            "Usa ← → o A D para navegar • ENTER para seleccionar • ESC para volver"
        )
    ...
    def _render_character_image(
        self, character_key: str, x: int, y: int, size: int = 120
    ):
        ...
        if image:
            ...
            pygame.draw.rect(
                self.screen, (150, 150, 150), (center_x - 30, center_y - 20, 60, 40), 2
            )
    ...
    def _render_character_stats(self, char_data: dict, x: int, y: int):
        ...
        stats_title = self.fonts["subtitle"].render(
    ...
    def _render_character_skills(self, char_data: dict, x: int, y: int):
        ...
        skills_title = self.fonts["subtitle"].render(
        ...
        for i, skill in enumerate(skills):
            skill_text = self.fonts["normal"].render(
                f"• {skill}", True, self.colors["text_secondary"]
            )
    ...
    def _render_buttons(self):
        ...
        for button_id, button in self.buttons.items():
            ...
            if button_id in ["back", "start"]:
                ...
                pygame.draw.rect(
                    self.screen, self.colors["panel_border"], button["rect"], 2
                )

                # Texto del botón
                button_text = self.fonts["normal"].render(
                    button["text"], True, self.colors["button_text"]
                )
                ...
                button_sprite = self.asset_manager.get_ui_button(
                ...
                if button_sprite:
                    ...
                    scaled_sprite = pygame.transform.scale(
                        button_sprite, (button["rect"].width, button["rect"].height)
                    )
                    ...
                    if button_sprite:
                        ...
                        # Texto de la flecha
                        button_text = self.fonts["title"].render(
                            button["text"], True, self.colors["text_highlight"]
                        )
    ...
    def _render_selected_info(self):
        ...
        if self.selected_key in self.character_data:
            ...
            info_rect = pygame.Rect(
                20, self.screen_height - 100, self.screen_width - 40, 40
            )
            ...
            ) // 8  # Aproximadamente 8 píxeles por carácter
            if len(desc_text) > max_chars:
                desc_text = desc_text[: max_chars - 3] + "..."

            desc_surface = self.fonts["small"].render(
                desc_text, True, self.colors["text"]
            )
            desc_rect = desc_surface.get_rect(
                midleft=(info_rect.x + 10, info_rect.centery)
            )
    ...
    def _get_character_image(self, character_key: str) -> Optional[pygame.Surface]:
        ...
        try:
            ...
            # Crear placeholder
            self.logger.warning(
                f"No se encontró imagen para {character_key}, usando placeholder"
            )
    ...
    def _init_buttons(self):
        ...
        self.buttons = {
            ...
            "arrow_right": {
                "rect": pygame.Rect(
                    self.screen_width - 110, button_y, button_width, button_height
                ),
                "text": "→",
            },
            "start": {
