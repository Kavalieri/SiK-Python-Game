...
class IntelligentAnimationManager:
    ...
    def load_character_animations(self, character_name: str) -> Dict[str, Dict]:
        ...
        for anim_type in animation_types:
            ...
            if real_frames:
                ...
                self.logger.info(
                    f"Animación cargada: {character_name}/{anim_type} - {len(real_frames)} frames a {optimal_fps} FPS"
                )
            else:
                self.logger.debug(
                    f"No se encontraron frames reales para {character_name}/{anim_type}"
                )

        self.logger.info(
            f"Cargadas {len(animations)} animaciones para {character_name}"
        )
        return animations
    ...
    def update_character_animation(self, character_name: str, delta_time: float):
        """
        Actualiza la animación de un personaje.

        Args:
            character_name: Nombre del personaje
            delta_time: Tiempo transcurrido
        """
        # Este método es para compatibilidad con el sistema anterior
        # La actualización real se hace en AnimationPlayer
        pass

    def set_animation_state(self, character_name: str, animation_state: str):
        """
        Establece el estado de animación de un personaje.
        ...
        """
        # Este método es para compatibilidad con el sistema anterior
        # La gestión de estados se hace en AnimationPlayer
        pass
    ...
    def get_current_sprite(
        ...
        Returns:
            Sprite actual o None si no está disponible
        """
        # Crear un reproductor temporal para obtener el frame actual
...
class AnimationPlayer:
    ...
    def __init__(
        self,
        manager: IntelligentAnimationManager,
        character_name: str,
