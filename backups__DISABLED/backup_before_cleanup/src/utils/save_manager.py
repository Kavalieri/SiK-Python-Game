...
class SaveManager:
	...
	def create_new_save(self, save_file_number: int) -> bool:
		...
		if not 1 <= save_file_number <= self.max_save_files:
			self.logger.error(f"Número de archivo de guardado inválido: {save_file_number}")
			return False
	...
	def load_save(self, save_file_number: int) -> Optional[Dict[str, Any]]:
		...
		try:
			...
			self.logger.error(f"Error al cargar archivo de guardado {save_file_number}: {e}")
			return None
	...
	def delete_save(self, save_file_number: int) -> bool:
		...
		try:
			...
			self.logger.error(f"Error al eliminar archivo de guardado {save_file_number}: {e}")
			return False
	...
	def export_save_debug(self, save_file_number: int, output_path: str) -> bool:
		...
		try:
			...
			self.logger.info(f"Archivo de guardado {save_file_number} exportado para debug: {output_path}")
			return True
