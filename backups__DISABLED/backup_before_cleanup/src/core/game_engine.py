...
class GameEngine:
    ...
    def _setup_scene_transitions(self):
        ...
        welcome_scene.menu_manager.add_callback(
            "main", lambda: self.scene_manager.change_scene("main_menu")
        )

        # Callbacks para la escena del menú principal
        main_menu_scene = self.scene_manager.scenes["main_menu"]
        main_menu_scene.menu_manager.add_callback(
            "new_game", lambda: self.scene_manager.change_scene("character_select")
        )
        main_menu_scene.menu_manager.add_callback(
            "continue_game", lambda: self.scene_manager.change_scene("game")
        )
        main_menu_scene.menu_manager.add_callback(
            "back_to_previous", lambda: self.scene_manager.change_scene("welcome")
        )
        ...
        pause_scene.menu_manager.add_callback(
            "resume_game", lambda: self.scene_manager.change_scene("game")
        )
        pause_scene.menu_manager.add_callback(
            "main_menu", lambda: self.scene_manager.change_scene("main_menu")
        )
