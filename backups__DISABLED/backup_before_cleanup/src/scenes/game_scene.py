...
class GameScene(Scene):
    ...
    def render(self):
        ...
        # Renderizar enemigos usando el nuevo sistema
        camera_offset = (self.camera.x, self.camera.y)
        self.enemy_manager.render(self.screen, camera_offset)
    ...
    def _spawn_enemy(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Enemigo {enemy_config.name} generado en ({x}, {y}) - Oleada {self.wave_number}"
            )
        except Exception as e:
    ...
    def _select_enemy_for_wave(self):
        ...
        if self.wave_number <= 3:
            ...
            return (
                EnemyTypes.get_random_by_rarity(EnemyRarity.NORMAL)
                or EnemyTypes.ZOMBIE_NORMAL
            )
            ...
            if rand < 0.8:
                ...
                return (
                    EnemyTypes.get_random_by_rarity(EnemyRarity.RARE)
                    or EnemyTypes.ZOMBIE_RARE
                )
            ...
            if rand < 0.6:
                ...
                return (
                    EnemyTypes.get_random_by_rarity(EnemyRarity.ELITE)
                    or EnemyTypes.ZOMBIE_ELITE
                )
        else:
            ...
            if rand < 0.5:
                ...
                return (
                    EnemyTypes.get_random_by_rarity(EnemyRarity.LEGENDARY)
                    or EnemyTypes.ZOMBIE_LEGENDARY
                )
    ...
    def _check_wave_completion(self):
        ...
        if (
            ...
            and not self.wave_completed
        ):
            self.wave_completed = True
            self.wave_number += 1
            self.enemies_spawned_this_wave = 0
            self.enemies_per_wave = min(
                10 + self.wave_number * 2, 50
            )  # Máximo 50 enemigos por oleada
            self.logger.info(
                f"¡Oleada {self.wave_number - 1} completada! Mostrando menú de mejoras"
            )
            # Mostrar menú de mejoras
            if hasattr(self, "hud") and hasattr(self.hud, "menu_manager"):
                self.hud.menu_manager.show_menu("upgrade")
                self.hud.menu_manager.update_upgrade_menu(
                    self.player.stats.upgrade_points
                )
                self.hud.menu_manager.add_callback(
                    "continue_after_upgrade", self._on_continue_after_upgrade
                )
                self.hud.menu_manager.add_callback(
                    "upgrade_speed", lambda: self.player.upgrade_stat("speed", 10)
                )
                self.hud.menu_manager.add_callback(
                    "upgrade_damage", lambda: self.player.upgrade_stat("damage", 15)
                )
                self.hud.menu_manager.add_callback(
                    "upgrade_health", lambda: self.player.upgrade_stat("health", 20)
                )
                self.hud.menu_manager.add_callback(
                    "upgrade_shield", lambda: self.player.upgrade_stat("shield", 25)
                )
    ...
    def _check_collisions(self):
        ...
        for projectile in self.projectiles[:]:
            enemies_in_range = self.enemy_manager.get_enemies_in_range(
            ...
            for enemy in enemies_in_range:
                if self._check_collision(projectile, enemy):
                    ...
                    # Añadir puntuación si el enemigo muere
                    if enemy.is_dead:
                        self.game_state.add_score(10)  # Puntuación base
                        self.logger.debug(
                            f"Enemigo eliminado. Puntuación: {self.game_state.score}"
                        )
        ...
        if self.player:
            ...
            for enemy in enemies_in_range:
                if self._check_collision(self.player, enemy):
                    # Aplicar daño al jugador si el enemigo puede atacar
                    if enemy.is_attack_ready():
                        damage = enemy.damage
                        if self.player.take_damage(damage):
                            self.game_state.lose_life()
                            self.logger.debug(
                                f"Jugador dañado. Vidas: {self.game_state.lives}"
                            )
