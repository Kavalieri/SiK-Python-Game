...
class Player(Entity):
    ...
    def _update_animation(self, delta_time: float):
        ...
        if new_animation_state != self.current_animation_state:
            ...
            self.animation_manager.set_animation_state(
                self.character_name, new_animation_state
            )

        # Actualizar la animación actual
        self.animation_manager.update_character_animation(
            self.character_name, delta_time
        )
    ...
    def _update_sprite(self):
        ...
        try:
            ...
            current_sprite = self.animation_manager.get_current_sprite(
                self.character_name
            )
    ...
    def heal(self, amount: float):
        ...
        super().heal(amount)
        self.logger.debug(
            f"Jugador curado {amount} puntos. Vida actual: {self.stats.health}"
        )
    ...
    def add_shield(self, amount: float):
        ...
        if added_shield > 0:
            self.logger.debug(
                f"Escudo añadido: {added_shield}. Escudo actual: {self.stats.shield}"
            )
    ...
    def add_upgrade_points(self, amount: int):
        ...
        self.logger.debug(
            f"Puntos de mejora añadidos: {amount}. Total: {self.stats.upgrade_points}"
        )
    ...
    def apply_powerup(self, powerup_effect: PowerupEffect):
        ...
        if powerup_effect.duration > 0:
            ...
            self.logger.info(
                f"Powerup {powerup_effect.type.value} aplicado por {powerup_effect.duration}s"
            )
        else:
            # Efecto instantáneo
            if powerup_effect.type == PowerupType.HEALTH:
                self.stats.health = min(
                    self.stats.max_health, self.stats.health + powerup_effect.value
                )
