...
def test_game_functionality():
    ...
    try:
        ...
        has_critical_errors = any(any(critical in error for critical in critical_errors) for error in errors)
        ...
        if "Motor del juego inicializado" in stdout or "Motor del juego inicializado" in stderr:
            print("✅ Motor del juego inicializado correctamente")
        else:
            print("⚠️  No se detectó inicialización del motor")
        
        if "Sistema de logging inicializado" in stdout or "Sistema de logging inicializado" in stderr:
        ...
        callback_errors = [error for error in errors if 'Callback no encontrado' in error]
        if callback_errors:
        ...
        if sprite_errors:
            print(f"⚠️  {len(sprite_errors)} errores de sprites (placeholders generados)")
