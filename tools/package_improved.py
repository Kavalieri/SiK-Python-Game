...
class UniversalPackager:
    ...
    def _load_config(self) -> Dict:
        ...
        config = {
            ...
            "assets_dir": "assets",
            "config_dir": "config",
            "exclude_dirs": ["__pycache__", ".git", ".pytest_cache", "tests", "scripts", "docs", "logs", "saves", "backups"],
            "exclude_files": [".gitignore", "README.md", "CHANGELOG.md", "requirements.txt", "pyproject.toml"],
            "additional_data": [],
            "hidden_imports": [],
            "console": False,
            "onefile": True,
    ...
    def build_pyinstaller_command(self, version: str) -> List[str]:
        ...
        if assets_dir.exists():
            command.append(f"--add-data={assets_dir}{data_sep}{self.config['assets_dir']}")
        
        if config_dir.exists():
            command.append(f"--add-data={config_dir}{data_sep}{self.config['config_dir']}")
    ...
    def _create_zip_package(self, release_dir: Path, dist_dir: Path, version: str):
        ...
        with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
            # Añadir ejecutable
            exe_name = f"{self.config['project_name']}_v{version}.exe" if sys.platform == "win32" else f"{self.config['project_name']}_v{version}"
