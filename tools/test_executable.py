...
def find_latest_executable():
    ...
    version_dirs = [d for d in releases_dir.iterdir() if d.is_dir() and d.name.startswith("v")]
...
def test_executable_basic(exe_path):
    ...
    try:
        ...
        if file_size < 1024*1024:  # Menos de 1MB
            logger.warning("El ejecutable parece ser muy pequeño, podría estar corrupto")
    ...
    try:
        ...
        if process.poll() is None:
            ...
            try:
                ...
                logger.warning("⚠️ El proceso no terminó en 5 segundos, forzando terminación")
                process.kill()
                return True
        else:
            # El proceso terminó prematuramente
            stdout, stderr = process.communicate()
            logger.error(f"❌ El ejecutable terminó prematuramente")
...
def main():
    ...
    if tests_passed == total_tests:
        logger.info("🎉 ¡Todas las pruebas pasaron! El ejecutable está listo para distribución.")
        return True
    else:
        logger.error(f"⚠️ {total_tests - tests_passed} prueba(s) fallaron. Revisar el ejecutable.")
        return False
