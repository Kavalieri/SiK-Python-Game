...
class RenderTester:
	...
	def test_loading_bar_mechanics(self) -> bool:
		...
		try:
			...
			if loading_scene.progress == 100:
				...
				self.logger.error(f"❌ Progreso final incorrecto: {loading_scene.progress}")
				return False
	...
	def test_scene_transitions(self) -> bool:
		...
		try:
			...
			if scene_manager.current_scene is None:
				...
				self.logger.warning(f"⚠️  Escena inicial inesperada: {scene_manager.current_scene}")
	...
	def run_all_tests(self) -> None:
		...
		print(f"📈 Ratio éxito: {self.tests_passed/(self.tests_passed+self.tests_failed)*100:.1f}%")
