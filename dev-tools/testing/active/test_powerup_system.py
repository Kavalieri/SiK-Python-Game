...
from src.entities.powerup import Powerup, PowerupEffect, PowerupType
...
def test_powerup_system():
    ...
    damage_effect = PowerupEffect(
        type=PowerupType.DAMAGE, duration=10.0, value=2.0, description="Aumenta el daño"
    )
    ...
    health_effect = PowerupEffect(
        type=PowerupType.HEALTH, duration=0.0, value=50.0, description="Restaura vida"
    )

    original_health = player.stats.health
    player.apply_powerup(health_effect)
    print(
        f"✅ Powerup de vida aplicado - Vida: {original_health} -> {player.stats.health}"
    )
