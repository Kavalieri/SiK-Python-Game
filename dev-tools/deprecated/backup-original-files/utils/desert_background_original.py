        ...
        for y in range(int(sky_height)):
            ...
            if ratio < 0.5:
                ...
                color = self._interpolate_color(
                    self.colors["sky_top"], self.colors["sky_mid"], local_ratio
                )
            else:
                # Segunda mitad: de sky_mid a sky_bottom
                local_ratio = (ratio - 0.5) * 2
                color = self._interpolate_color(
                    self.colors["sky_mid"], self.colors["sky_bottom"], local_ratio
                )
    ...
    def _render_dune_effects(
        ...
        if len(offset_points) > 2:
            ...
            if len(highlight_points) > 2:
                pygame.draw.polygon(
                    screen, self.colors["dune_highlight"], highlight_points
                )
        ...
        if self.time % 2 < 1:
            ...
            for i in range(0, self.screen_width, 50):
                ...
                if heat_intensity > 10:
                    ...
                    heat_surface = pygame.Surface(
                        (2, self.screen_height), pygame.SRCALPHA
                    )
                    pygame.draw.line(
                        heat_surface, heat_color, (0, 0), (0, self.screen_height)
                    )
    ...
    def _render_wind_effect(
        ...
        for start, end in wind_lines:
            ...
            wind_surface = pygame.Surface(
                (abs(end[0] - start[0]) + 2, abs(end[1] - start[1]) + 2),
                pygame.SRCALPHA,
            )
            pygame.draw.line(
                wind_surface,
