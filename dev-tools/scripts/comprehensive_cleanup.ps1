...
function Invoke-VSCodeCleanup {
	...
	if (Test-VSCodeRunning) {
		...
		if ($CleanupLevel -eq "shutdown") {
			$aggressiveScript = ".\dev-tools\scripts\vscode_aggressive_cleanup.ps1"
			if (Test-Path $aggressiveScript) {
				try {
					Write-Log "Ejecutando limpieza AGRESIVA de VS Code para shutdown..." "WARN"
					& $aggressiveScript -Level "shutdown" -Force
					Write-Log "Limpieza agresiva de VS Code completada" "SUCCESS"
					return
				} catch {
					Write-Log "Error en limpieza agresiva: $($_.Exception.Message)" "ERROR"
				}
			}
		}

		# Verificar si existe el script de SendKeys estándar
		$SendKeysScript = ".\dev-tools\scripts\vscode_cleanup_sendkeys.ps1"
		if (Test-Path $SendKeysScript) {
			try {
				Write-Log "Ejecutando limpieza estándar de VS Code con SendKeys..." "INFO"
				...
				if ($CleanupLevel -eq "complete") {
					$aggressiveScript = ".\dev-tools\scripts\vscode_aggressive_cleanup.ps1"
					if (Test-Path $aggressiveScript) {
						Write-Log "Aplicando limpieza agresiva adicional..." "INFO"
...
# Función de limpieza de archivos no rastreados por Git
function Clear-UntrackedFiles {
	Write-Log "Iniciando limpieza de archivos no rastreados..." "INFO"
	...
	try {
		...
		if ($untrackedFiles.Count -eq 0) {
			Write-Log "No hay archivos no rastreados para limpiar" "INFO"
			return
		}

		Write-Log "Encontrados $($untrackedFiles.Count) archivos no rastreados" "INFO"

		# Patrones de archivos a eliminar automáticamente
		$autoDeletePatterns = @(
			"*_new.py",
			"*_old.py",
			"*_backup.py",
		...
		foreach ($file in $untrackedFiles) {
			...
			# Verificar si coincide con algún patrón de auto-eliminación
			foreach ($pattern in $autoDeletePatterns) {
			...
			if ($shouldDelete) {
				try {
					if (Test-Path $file) {
						...
						Write-Log "Archivo no rastreado eliminado: $file" "SUCCESS"
					}
				} catch {
					Write-Log "Error eliminando archivo no rastreado $file : $($_.Exception.Message)" "WARN"
				}
			} else {
				Write-Log "Archivo no rastreado conservado: $file" "INFO"
			}
		}

	} catch {
		Write-Log "Error en limpieza de archivos no rastreados: $($_.Exception.Message)" "ERROR"
	}
}
...
function Invoke-ComprehensiveCleanup {
	...
	try {
		...
		# Limpieza de archivos no rastreados (siempre)
		Clear-UntrackedFiles
		...
		if ($Level -eq "shutdown" -or $PreShutdown) {
			Invoke-VSCodeCleanup -CleanupLevel "shutdown"
		} else {
