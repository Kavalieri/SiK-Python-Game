...
def package_project(version):
    ...
    with zipfile.ZipFile(zip_filename, "w", zipfile.ZIP_DEFLATED) as zipf:
        ...
        exe_name = (
            f"PyGame_v{version}.exe"
            if sys.platform == "win32"
            else f"PyGame_v{version}"
        )
        ...
        if os.path.exists(exe_path):
            ...
            print(
                f"Advertencia: El ejecutable {exe_path} no se encontró para el archivo ZIP."
            )
        ...
        for root, _, files in os.walk("assets"):
            for file in files:
                ...
                zipf.write(
                    file_path, os.path.relpath(file_path, os.path.dirname("assets"))
                )
